...
acoustic_features_path = '../features/IEMOCAP/acoustic_features_VGGish'
new_acoustic_features_path = '../features/IEMOCAP/acoustic_features_VGGish_normalized'
...
for session in os.listdir(acoustic_features_path):
	for dialog in os.listdir(os.path.join(acoustic_features_path, session)):
		print(dialog)
		for sentence in os.listdir(os.path.join(acoustic_features_path, session, dialog)):
			feature = np.load(os.path.join(acoustic_features_path, session, dialog, sentence))
			gender = sentence[-8]
	...
	for dialog in os.listdir(os.path.join(acoustic_features_path, session)):
		for sentence in os.listdir(os.path.join(acoustic_features_path, session, dialog)):
			...
			output_file = os.path.join(new_acoustic_features_path, session, dialog)
